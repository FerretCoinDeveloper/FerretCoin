...
bool parseFerretCoinURI(const QUrl &uri, SendCoinsRecipient *out)
{
    ...
    for (QList<QPair<QString, QString> >::iterator i = items.begin(); i != items.end(); i++)
    {
        ...
        else if (i->first == "amount")
        {
            if(!i->second.isEmpty())
            {
                if(!FerretCoinUnits::parse(FerretCoinUnits::FRTC, i->second, &rv.amount))
                {
                    return false;
                }
            }
...
bool parseFerretCoinURI(QString uri, SendCoinsRecipient *out)
{
    ...
    if(uri.startsWith("ferretcoin://", Qt::CaseInsensitive))
    {
...
LSSharedFileListItemRef findStartupItemInList(LSSharedFileListRef list, CFURLRef findUrl)
{
    // loop through the list of startup items and try to find the FerretCoin app
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if(fAutoStart && !foundItem) {
        // add FerretCoin app to startup item list
