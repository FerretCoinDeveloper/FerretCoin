...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nDisplayUnit"))
        settings.setValue("nDisplayUnit", FerretCoinUnits::FRTC);
    ...
    if (!settings.contains("nAnonymizeFerretCoinAmount"))
        settings.setValue("nAnonymizeFerretCoinAmount", 1000);
    nAnonymizeFerretCoinAmount = settings.value("nAnonymizeFerretCoinAmount").toLongLong();
    if (settings.contains("nMNengineRounds"))
        SoftSetArg("-mnenginerounds", settings.value("nMNengineRounds").toString().toStdString());
    if (settings.contains("nAnonymizeFerretCoinAmount"))
        SoftSetArg("-anonymizeFerretCoinamount", settings.value("nAnonymizeFerretCoinAmount").toString().toStdString());
...
QVariant OptionsModel::data(const QModelIndex & index, int role) const
{
    if(role == Qt::EditRole)
    {
        ...
        case AnonymizeFerretCoinAmount:
            return QVariant(nAnonymizeFerretCoinAmount);
        case UseDarkTheme:
...
bool OptionsModel::setData(const QModelIndex & index, const QVariant & value, int role)
{
    ...
    if(role == Qt::EditRole)
    {
        ...
        case MNengineRounds:
            ...
            break;
        case AnonymizeFerretCoinAmount:
            nAnonymizeFerretCoinAmount = value.toInt();
            settings.setValue("nAnonymizeFerretCoinAmount", nAnonymizeFerretCoinAmount);
            emit AnonymizeFerretCoinAmountChanged(nAnonymizeFerretCoinAmount);
            break;
        default:
            break;
        }
