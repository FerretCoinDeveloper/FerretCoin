...
QString QRCodeDialog::getURI()
{
    ...
    if (ui->chkReqPayment->isChecked())
    {
        ...
        {
            // even if we allow a non FRTC unit input in lnReqAmount, we generate the URI with FRTC as unit (as defined in BIP21)
            ret += QString("?amount=%1").arg(FerretCoinUnits::format(FerretCoinUnits::FRTC, ui->lnReqAmount->value()));
            paramCount++;
        }
        else
        {
