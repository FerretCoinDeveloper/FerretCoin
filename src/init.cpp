...
void Shutdown()
{
    ...
    RenameThread("FerretCoin-shutoff");
...
std::string HelpMessage()
{
    ...
    strUsage += "  -conf=<file>           " + _("Specify configuration file (default: FerretCoin.conf)") + "\n";
    strUsage += "  -pid=<file>            " + _("Specify pid file (default: FerretCoind.pid)") + "\n";
    ...
    strUsage += "\n" + _("SSL options: (see the FerretCoin Wiki for SSL setup instructions)") + "\n";
...
/** Sanity checks
 *  Ensure that FerretCoin is running in a usable environment with all
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    // Sanity check
    if (!InitSanityCheck())
        return InitError(_("Initialization sanity check failed. FerretCoin is shutting down."));
    ...
    if (!lock.try_lock())
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. FerretCoin is probably already running."), strDataDir));

    if (GetBoolArg("-shrinkdebugfile", !fDebug))
        ShrinkDebugFile();
    LogPrintf("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
    LogPrintf("FerretCoin version %s (%s)\n", FormatFullVersion(), CLIENT_DATE);
