...
uint32_t FerretCoinChecksum(uint8_t* p, uint32_t nBytes)
{
    if (!p || nBytes == 0)
        return 0;
...
{
    uint32_t checksum = FerretCoinChecksum(&data[0], data.size());
...
bool VerifyChecksum(const data_chunk& data)
{
    ...
    return FerretCoinChecksum((uint8_t*)&data[0], data.size()-4) == checksum;
};
