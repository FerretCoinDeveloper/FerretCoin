...
//
// Bootup the masternode, look for a 2,000,000 FRTC input and register on the network
//
void CActiveMasternode::ManageStatus()
{
    ...
    if(status == MASTERNODE_NOT_PROCESSED) {
        ...
        if(GetMasterNodeVin(vin, pubKeyCollateralAddress, keyCollateralAddress)) {
            ...
            /* donations are not supported in FerretCoin.conf */
...
{
    CFerretCoinAddress address(collateralAddress);
