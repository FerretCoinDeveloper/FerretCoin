...
bool CFerretCoinAddress::Set(const CKeyID &id) {
    SetData(Params().Base58Prefix(CChainParams::PUBKEY_ADDRESS), &id, 20);
    return true;
}

bool CFerretCoinAddress::Set(const CScriptID &id) {
    SetData(Params().Base58Prefix(CChainParams::SCRIPT_ADDRESS), &id, 20);
    return true;
}

bool CFerretCoinAddress::Set(const CTxDestination &dest) {
    return boost::apply_visitor(CFerretCoinAddressVisitor(this), dest);
}

bool CFerretCoinAddress::IsValid() const {
...
CTxDestination CFerretCoinAddress::Get() const {
    if (!IsValid())
...
bool CFerretCoinAddress::GetKeyID(CKeyID &keyID) const {
...
bool CFerretCoinAddress::IsScript() const {
    return IsValid() && vchVersion == Params().Base58Prefix(CChainParams::SCRIPT_ADDRESS);
}

void CFerretCoinSecret::SetKey(const CKey& vchSecret) {
...
CKey CFerretCoinSecret::GetKey() {
    CKey ret;
    ret.Set(&vchData[0], &vchData[32], vchData.size() > 32 && vchData[32] == 1);
    return ret;
}

bool CFerretCoinSecret::IsValid() const {
    bool fExpectedFormat = vchData.size() == 32 || (vchData.size() == 33 && vchData[32] == 1);
    bool fCorrectVersion = vchVersion == Params().Base58Prefix(CChainParams::SECRET_KEY);
    return fExpectedFormat && fCorrectVersion;
}

bool CFerretCoinSecret::SetString(const char* pszSecret) {
    return CBase58Data::SetString(pszSecret) && IsValid();
}

bool CFerretCoinSecret::SetString(const std::string& strSecret) {
